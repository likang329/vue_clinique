<template>
	<div class="home">
	  ...
	  <div class="product_list">
	  	<ul class="clearfix">
	  		<li v-for="(item, index) in homeJson" :key="item.id">
	  			<div class="product_img">
	  				<img :src="item.img"/>
	  			</div>
	  			<div class="text_content">
	  				<h4 class="name" v-html="item.name"></h4>
	  				<p>{{homeData.products[index].usedNum}}个密友用过</p>
	  				<span>看口碑</span>
	  			</div>
	  		</li>
	  	</ul>
...
<script>
	...
	export default {
	  ...
	  methods: {
	    ...
	    indexData () {
	    	var _this = this;
	  		this.$http.get(API_PROXY + 'https://misc.kimiss.com/common/wxapi/clinique?rd=' + 1001)
	  		.then(res => {
